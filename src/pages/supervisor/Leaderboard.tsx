...
export default function SupervisorLeaderboard() {
  ...
  const fetchLeaderboardData = async () => {
    try {
      const [internsSnap, submissionsSnap, gradesSnap, groupsSnap] = await Promise.all([
      ...
      if (internsSnap.exists()) {
        ...
        supervisorInternIds.forEach((internId) => {
  ...
  const isTopPerformer = (rank: number) => rank <= 3;
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <h1 className="text-3xl font-bold text-gray-900 mb-2">🏆 My Interns Leaderboard</h1>
        <p className="text-gray-600">Performance rankings of your supervised interns</p>
        <div className="flex items-center justify-center space-x-2 mt-2">
          <Users className="h-5 w-5 text-blue-500" />
          <span className="text-sm text-gray-600">{leaderboard.length} interns</span>
        </div>
      </motion.div>
      ...
      {leaderboard.length === 0 ? (
        <Card className="p-12 text-center">
          <div className="text-gray-500">
            ...
            <p>No interns assigned to you yet, or no activity to rank.</p>
          </div>
        </Card>
      ) : (
        <>
          ...
          <div className="space-y-4">
            {leaderboard.map((entry, index) => {
              const isTop3 = isTopPerformer(index + 1);
              ...
              return (
                ...
                >
                  <Card className={`p-6 ${isTop3 ? 'ring-2 ring-yellow-200 bg-gradient-to-r from-yellow-50 to-orange-50' : ''}`}>
                          ...
                          <div className="w-12 h-12">
                            <CircularProgressbar
                              ...
                              styles={buildStyles({
                                textSize: '20px',
                                textColor: '#1f2937',
                                pathColor: isTop3 ? '#f59e0b' : '#3b82f6',
                                trailColor: '#e5e7eb',
                              })}
                            />
                          </div>
            ...
            >
              <Card className="p-6 bg-gradient-to-r from-blue-50 to-teal-50 border-blue-200">
                <div className="flex items-center space-x-2 mb-4">
                  ...
                  <h3 className="text-lg font-semibold text-blue-900">Team Performance Summary</h3>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="text-center">
                    ...
                    <div className="text-sm text-green-800">Team Average Grade</div>
                  </div>
                  <div className="text-center">
